...
// Last update Mon Feb  6 11:22:45 2017 Riamon Vincent
//

#include "XOR-4030.hpp"

XOR4030::XOR4030(__attribute__((unused))nts::Tristate val) {
  for (int i = 0; i < 14; i++)
    _pins[i] = NULL;
...
void XOR4030::SetLink(size_t this_pin, nts::IComponent& comp, size_t target_pin) {
  if (this_pin >= 14) {
    std::stringstream ss;
...
nts::Tristate XOR4030::xor_gate(size_t first_pin, size_t second_pin) const {
  return (static_cast<nts::Tristate>(first_pin ^ second_pin));
}

nts::Tristate XOR4030::Compute(size_t this_pin) {
  if (this_pin >= 14) {
    std::stringstream ss;
...
nts::Tristate XOR4030::calcInput(size_t this_pin) {
  if (!_pins[this_pin - 1])
...
nts::Tristate XOR4030::calcOutput(size_t this_pin) {
  size_t first_pin = 0;
  size_t second_pin = 0;
